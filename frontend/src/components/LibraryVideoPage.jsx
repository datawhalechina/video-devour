...
import { Download, FileDown, Loader2, Clock } from "lucide-react";
...
function LibraryVideoPage() {
  ...
  const loadVideo = async () => {
    const res = await fetch(
      `/api/library/video/${encodeURIComponent(videoKey)}`,
    );
    if (!res.ok) {
  ...
  const activeScope = useMemo(() => {
    const ready = SCOPE_ORDER.filter((s) =>
      activeRun?.articles?.some((a) => a.scope === s),
    );
    if (scope && (ready.includes(scope) || GENERATABLE.includes(scope)))
      return scope;
  ...
  const currentQuery =
    activeRun && activeScope
      ? `/api/library/article/${activeRun.doc_id}/${activeScope}/download` +
        (activeRun.run_id
          ? `?run_id=${encodeURIComponent(activeRun.run_id)}`
          : "")
      : "";
  const downloadHref = currentQuery
    ? `${currentQuery}${currentQuery.includes("?") ? "&" : "?"}fmt=md`
    : "";
  const pdfHref = currentQuery
    ? `${currentQuery}${currentQuery.includes("?") ? "&" : "?"}fmt=pdf`
    : "";

  return (
    <div className="vd-page vd-library-reader libraryvideopage">
      <header className="vd-reader-topbar">
        <div className="vd-reader-topbar-inner">
          ...
          <>
            ...
            <div className="video-hero">
              ...
              <div className="video-hero-meta">
                ...
                {activeRun && (
                  <span>
                    {activeRun.version_label} ·{" "}
                    {new Date(activeRun.created_at).toLocaleString("zh-CN")}
                  </span>
                )}
                {video.source_url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="truncate max-w-[23.75rem] hover:text-primary-600"
                  >
                    {video.source_url}
                  </a>
                )}
            ...
            <ContentNavigation
              value={activeScope}
              disabledKeys={[...Object.keys(generating), ...SCOPE_ORDER.filter(key => !GENERATABLE.includes(key) && !activeRun?.articles?.some(article => article.scope === key))]}
              onChange={(key) =>
                LEARNING_KEYS.includes(key)
                  ? navigate(
                      `/learn/${encodeURIComponent(activeRun.doc_id)}/${key}`,
                      {
                        state: {
                          reader: {
                            path: `/library/video/${encodeURIComponent(videoKey)}`,
                  ...
                  {article.version_label && (
                    <span className="library-tag is-version">
                      ...
                      {article.version_count > 1
                        ? ` / 共 ${article.version_count} 版`
                        : ""}
                    </span>
                  )}
                  {activeRun?.education_level && (
                    <span className="library-tag">
                      {activeRun.education_level}
                    </span>
                  )}
                </div>
                <ReadingLayout contentKey={article.content}>
