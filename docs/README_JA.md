...
- **ASR モード**：デフォルトは `オンライン`（クラウド、ダウンロード不要ですぐ使える）。ローカル `オフライン`（FunASR Paraformer）は約 2GB のモデル導入が必要で、切り替えるとセルフチェックと導入コマンドが表示されます（または `bash scripts/install_offline_asr.sh`）
...
### オンライン動画リンク処理（ビリビリ / YouTube / 抖音 / WeChat チャンネル）
...
- **抖音（Douyin）**：`douyin.com/video/{id}` と `v.douyin.com` 短縮リンクに対応（自動解決）。**ダウンロードにはログイン済み Cookie が必要**（「抖音 cookies」設定またはブラウザ Cookie の一括取得を利用）。キーワード検索は抖音側のログイン制限があるため、代わりに**トレンド話題の発見**を提供します。
...
- YouTube はボット判定あり：メタデータは oEmbed に自動フォールバック。ダウンロードには設定コンソールの「YouTube cookies」が必要（または `YTDLP_COOKIES_FILE` で cookies ファイルを指定）。設定ページの「ブラウザから Cookie を一括取得」で自動取得も可能です。ダウンロードには yt-dlp の EJS ソルバーと JS ランタイムも必要です（`uv sync` で `yt-dlp[default]` が入ります。Deno または Node 22 以上が必要）。PO Token サポートは任意で `bash scripts/install_yt_pot.sh`。設定ページの「ダウンロード環境セルフチェック」で一括確認できます
