...
- YouTube has bot checks: metadata falls back to oEmbed; downloads need the "YouTube cookies" setting in the console (or `YTDLP_COOKIES_FILE` pointing to a cookies file) — you can also grab them automatically via "Import cookies from browser" on the settings page. Downloads also need the yt-dlp EJS solver and a JS runtime (`uv sync` includes `yt-dlp[default]`; Deno or Node >=22 must be present), plus optional PO Token support via `bash scripts/install_yt_pot.sh`. Use "Download environment self-check" on the settings page to verify everything
