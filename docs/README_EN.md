...
- YouTube has bot checks: metadata falls back to oEmbed; downloads need the "YouTube cookies" setting in the console (or `YTDLP_COOKIES_FILE` pointing to a cookies file) — you can also grab them automatically via "Import cookies from browser" on the settings page. Some networks (datacenter/proxy IPs) get forced into SABR streaming; for those install PO Token support: `bash scripts/install_yt_pot.sh` (requires node), and keep yt-dlp up to date
